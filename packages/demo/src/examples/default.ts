...
type AppDb = dsl.DatabaseDefinition<
  {
    users: dsl.CollectionShape<User, { notifications: dsl.CollectionShape<Notification> }>
  },
  AppClaims
>

export default dsl
  .createAstRulesBuilder<AppDb>()
  .withHelpers(($, { def, arg }) => ({
    isSignedIn: def("isSignedIn", { body: () => $.hasPath($.request, "auth.uid") }),
    hasRole: def("hasRole", {
  ...
  .matches((match) => {
    match("users/{userId}", ({ allow, matches }, $) => {
      // rules
      allow("read", $.and($.isSignedIn(), $.request.auth.uid.eq($.params.userId)))
      allow(["create", "update", "delete"], $.request.auth.token.admin.eq(true))
      // sub-collection
      matches((match) => {
